...
cv = list(kfold.split(train_X, train_y))
...
class OrdinalWeightedError(nn.Module):
    ...
    def forward(self, input, target):
        ...
        return torch.sum(torch.abs(input_PCNL-target_PCNL)*target_weight)/torch.sum(target_weight)
...
import pickle
f = open('reg_class_OWAE_losses.txt', 'wb')
pickle.dump(losses_list, f)
f.close()
f = open('reg_class_OWAE_QWK.txt', 'wb')
pickle.dump(QWK_scores_list, f)
f.close()
f = open('reg_class_OWAE_CEM.txt', 'wb')
pickle.dump(CEM_scores_list, f)
f.close()
f = open('reg_class_OWAE_QWK2.txt', 'wb')
pickle.dump(QWK_scores2_list, f)
f.close()
f = open('reg_class_OWAE_CEM2.txt', 'wb')
pickle.dump(CEM_scores2_list, f)
f.close()
f = open('reg_class_OWAE_QWK3.txt', 'wb')
pickle.dump(QWK_scores3_list, f)
f.close()
f = open('reg_class_OWAE_CEM3.txt', 'wb')
pickle.dump(CEM_scores3_list, f)
f.close()
f = open('reg_class_OWAE_valid_preds.txt', 'wb')
pickle.dump(valid_preds_list, f)
f.close()
f = open('reg_class_OWAE_valid_outputs.txt', 'wb')
