...
class OrdinalWeightedError(nn.Module):
    ...
    def forward(self, input, target):
        PCNL_target = torch.tensor([self.PCNL_map[x.item()] for x in target]).to(DEVICE)
        weight_target = torch.tensor([self.weight_map[x.item()] for x in target]).to(DEVICE)
        return torch.sum(torch.abs(input-PCNL_target)*weight_target)/torch.sum(weight_target)
...
def train_fn(dataloader, model, criterion, optimizer, scheduler, device, epoch):
    ...
    train_loss = criterion(all_outputs_torch, all_labels_torch)
    return train_loss
...
def eval_fn(dataloader, model, criterion, device, epoch):
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(progress):
            ...
            outputs_PCNL = outputs.reshape(-1)
            PCNL_map = torch.tensor(list(criterion.PCNL_map.values())).to(DEVICE)
            preds = torch.abs(PCNL_map - outputs).argmin(dim=1).reshape(-1)
            del input_ids, attention_mask
            ...
            all_outputs_torch = torch.cat((all_outputs_torch, outputs.reshape(-1)), 0)
            all_outputs += outputs.tolist()
            all_outputs_PCNL += outputs_PCNL.tolist()
            
            del labels, outputs, preds
            progress.set_postfix()

        valid_loss = criterion(all_outputs_torch, all_labels_torch)
    all_preds2 = greedy_predict(all_outputs_PCNL, all_labels)
    distribution = list(criterion.PCNL_map.values())
    thresholds = [(distribution[i]+distribution[i+1])/2 for i in range(len(distribution)-1)]
    ...
    return valid_loss, QWK_score, QWK_score2, QWK_score3, CEM_score, CEM_score2, CEM_score3, all_preds, all_outputs_PCNL
...
def trainer(fold, df):
    ...
    for epoch in range(EPOCHS):
        ...
        valid_loss, QWK_score, QWK_score2, QWK_score3, CEM_score, CEM_score2, CEM_score3, valid_pred, valid_output_PCNL = eval_fn(valid_dataloader, model, criterion, DEVICE, epoch)
        ...
        valid_outputs_PCNL.append(valid_output_PCNL)

    return valid_losses, QWK_scores, QWK_scores2, QWK_scores3, CEM_scores, CEM_scores2, CEM_scores3, valid_preds, valid_outputs_PCNL
...
CEM_scores3_list = []
valid_preds_list = []
valid_outputs_PCNL_list = []
oof_pred = np.zeros(len(train_y))
oof_outputs_PCNL = np.zeros(len(train_y))
...
for fold in range(NUM_SPLITS):
    ...
    valid_losses, QWK_scores, QWK_scores2, QWK_scores3, CEM_scores, CEM_scores2, CEM_scores3, valid_preds, valid_outputs_PCNL = trainer(fold, df)
    ...
    valid_outputs_PCNL_list.append(valid_outputs_PCNL)
...
import pickle
f = open('reg_PCNL_OWAE_losses.txt', 'wb')
pickle.dump(losses_list, f)
f.close()
f = open('reg_PCNL_OWAE_QWK.txt', 'wb')
pickle.dump(QWK_scores_list, f)
f.close()
f = open('reg_PCNL_OWAE_CEM.txt', 'wb')
pickle.dump(CEM_scores_list, f)
f.close()
f = open('reg_PCNL_OWAE_QWK2.txt', 'wb')
pickle.dump(QWK_scores2_list, f)
f.close()
f = open('reg_PCNL_OWAE_CEM2.txt', 'wb')
pickle.dump(CEM_scores2_list, f)
f.close()
f = open('reg_PCNL_OWAE_QWK3.txt', 'wb')
pickle.dump(QWK_scores3_list, f)
f.close()
f = open('reg_PCNL_OWAE_CEM3.txt', 'wb')
pickle.dump(CEM_scores3_list, f)
f.close()
f = open('reg_PCNL_OWAE_valid_preds.txt', 'wb')
pickle.dump(valid_preds_list, f)
f.close()
f = open('reg_PCNL_OWAE_valid_outputs_PCNL.txt', 'wb')
pickle.dump(valid_outputs_PCNL_list, f)
f.close()
